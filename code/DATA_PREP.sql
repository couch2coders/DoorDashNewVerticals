...
*/
SELECT 
  DELIVERY_UUID AS DELIV_ID
  , FORMAT_TIMESTAMP('%F %T', TIMESTAMP(DATETIME(DELIV_CREATED_AT), 'America/New_York')) AS EST_TS
  ,DATE(TIMESTAMP(DATETIME(DELIV_CREATED_AT), 'America/New_York')) AS EST_DT
  ,EXTRACT(HOUR FROM TIMESTAMP(DATETIME(DELIV_CREATED_AT), 'America/New_York')) AS HR
  ,EXTRACT(MINUTE FROM TIMESTAMP(DATETIME(DELIV_CREATED_AT), 'America/New_York')) AS MINUTE
  ,DELIV_STORE_NAME AS STORE
  ...
  ,DELIV_CLAT
  ,DELIV_IS_20_MIN_LATE AS LATE_FLG
  ,DATE_DIFF(DELIV_CANCELLED_AT,DELIV_CREATED_AT, MINUTE) AS MINUTES_TO_CANCEL
  ,DELIV_CANCELLED_AT
  ,IF(DELIV_CANCELLED_AT IS NOT NULL, 1,0) AS CANCEL_FLG
  ,MAX(CAST(DELIV_MISSING_INCORRECT_REPORT AS INT64)) AS COMPLAINT
  ,SUM(WAS_REQUESTED) AS REQ
  ,SUM(WAS_MISSING) AS MISS
  ,SUM(WAS_SUBBED) AS SUB
  ,SUM(WAS_FOUND) AS FOUND
  ,COUNT(*) AS N_ITEMS
FROM `couch2coding.DoorDash_Case_Study.NEW_VERTICALS` 
-- WHERE DELIV_CANCELLED_AT IS NOT NULL
GROUP BY ALL
order by CANCEL_FLG
-- GROUP BY ALL
-- HAVING T > 1
